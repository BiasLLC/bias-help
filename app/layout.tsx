      ...
      <body>
        <header className="fixed top-0 left-0 right-0 h-16 bg-white border-b border-gray-200 z-50">
          <div className="h-full px-6 flex items-center justify-between">
            <Link href="/" className="text-xl font-bold text-blue-600">BIAS Writer</Link>
        ...
        <div className="flex mt-16 min-h-screen">
          <aside className="fixed left-0 w-64 h-[calc(100vh-4rem)] bg-gray-50 border-r border-gray-200 overflow-y-auto">
            <nav className="p-6">
              <div className="mb-6">
                <input type="search" placeholder="Search..." className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:border-blue-500" />
              </div>
              <div className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-3">Contents</div>
              <ul className="space-y-1">
                {chapters.map((chapter) => (
                  <li key={chapter.id}>
                    <a href={'#' + chapter.id} className="block px-3 py-2 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-600 rounded-md transition">{chapter.title}</a>
                  </li>
                ))}
              </ul>
            </nav>
